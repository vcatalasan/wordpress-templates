<?php
/*
Plugin Name: WP-Cycle Plus Captions
Plugin URI: http://www.tech-squawkers.com/wp-cycle-plus-captions/
Description: Based on WP-Cycle with modifications by Chris Grab and includes changes by Toby Brommerich to get the caption to display properly with each image.
Version: 0.4.5
Author: Toby Brommerich
Author URI: http://www.tech-squawkers.com/who-is-toby/
...
*/
/*
	Initial Caption changes by Chris Grab
*/

/*****Change History*****/
/*
This Version of the plugin was modified again 8/1/2012 by Toby Brommerich
Moved the caption paragraph added by Chris Grab into the foreach loop for proper display and transition. 
Copied Alt tag varabile to caption paragraph. Commented out styles for #rotator so it could inhert the contents width and height so the caption would display below each image. 
*/
/* 
this version of the plugin was modified on April 27 2011 by chris grab: chris@chrisgrab.com
a field was added to the admin interface allowing a caption to be added
the caption is added to the ALT attribute of the image
the caption is also displayed via jQuery in a paragraph with the id "caption" so it can be styled
*/
...
function wp_cycle_handle_upload() {
	...
	if($width > $wp_cycle_settings['img_width'] || $height > $wp_cycle_settings['img_height']) {
		...
		if ( ! is_wp_error( $image ) ) {
			$image->resize($wp_cycle_settings['img_width'], $wp_cycle_settings['img_height'], true);
	...
	// UPDATE April 2011 by Chris Grab - added wp_cycle_image_caption to the array
...
function wp_cycle_images_update_check() {
	global $wp_cycle_images;
	if(isset( $wp_cycle_images['update'] ) && $wp_cycle_images['update'] == 'Added' || isset( $wp_cycle_images['update'] ) && $wp_cycle_images['update'] == 'Deleted' || isset( $wp_cycle_images['update'] ) && $wp_cycle_images['update'] == 'Updated') {
...
function wp_cycle_images_admin() { ?>
	...
	<h2><?php _e('WP-Cycle Plus Caption Images', 'wp_cycle'); ?></h2>
	...
	<table class="widefat fixed" cellspacing="0">
		...
		<?php foreach((array)$wp_cycle_images as $image => $data) : ?>
			<tr>
				...
				<?php if($data['thumbnail']):?><input type="hidden" name="wp_cycle_images[<?php echo $image; ?>][thumbnail]" value="<?php echo $data['thumbnail']; ?>" />
				<input type="hidden" name="wp_cycle_images[<?php echo $image; ?>][thumbnail_url]" value="<?php echo $data['thumbnail_url']; ?>" /><?php endif;?>
				...
				<td><input type="text" name="wp_cycle_images[<?php echo $image; ?>][wp_cycle_image_caption]" value="<?php echo $data['wp_cycle_image_caption']; ?>" size="35" /></td>
...
function wp_cycle_settings_admin() { ?>
	...
	<h2><?php _e('WP-Cycle Plus Caption Settings', 'wp-cycle'); ?></h2>
	<form method="post" action="options.php">
	...
	<table class="form-table">
		<tr><th scope="row">Enable Random Image Order</th>
			<td><input name="wp_cycle_settings[random]" type="checkbox" value="1" <?php checked('1', $options['random']); ?> /> <label for="wp_cycle_settings[random]">Check this box if you want to enable random image order</td>
		</td></tr>
...
function wp_cycle_images_validate($input) {
	foreach((array)$input as $key => $value) {
		if($key != 'update') {
			...
			// UPDATE added april 27 2011 by chris grab - sanitize caption before adding to DB
			if($value['wp_cycle_image_caption'])
			$input[$key]['wp_cycle_image_caption'] = wp_filter_nohtml_kses($value['wp_cycle_image_caption']);
		}
	}
	return $input;
}
...
function wp_cycle($args = array(), $content = null) {
	...
	foreach((array)$wp_cycle_images as $image => $data) {
		...
		// update april 27 2011 by chris grab -- added caption output to ALT attribute of image tag. is used again later by jquery function
		echo '<img src="'.$data['file_url'].'" width="'.$wp_cycle_settings['img_width'].'" height="'.$wp_cycle_settings['img_height'].'" class="'.$data['id'].'" alt="'. $data['wp_cycle_image_caption'] .'" />';
		
		if($data['image_links_to'])
			echo '</a>';
		//updated 8/1/2012 by Toby Brommerich -- Moved Caption inside foreach, copied caption variable from img alt
		echo '<p id="caption">'. $data['wp_cycle_image_caption'].'</p>';
		echo '</div>';
	}
...
function wp_cycle_scripts() {
	if(!is_admin())
	wp_enqueue_script('cycle', WP_CONTENT_URL.'/plugins/wp-cycle-plus-captions/jquery.cycle.all.min.js', array('jquery'), '', true);
}
